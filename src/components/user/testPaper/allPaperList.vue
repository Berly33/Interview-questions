<template>
  <div>
          <div class="main-back">
            ...
            <div class="record">
              <div class="record-children" v-for="item in detailData" :key="item">
                <div class="image-title" @click="toDetail(item.testpaperId)">
                  <img :src="item.subjectInfo.subjectPic" class="image-logo">
                  <div class="title-name">
                    <div>{{item.subjectInfo.subjectName}}</div>
                    <div class="title-on">{{item.testpaperNumber}}</div>
                  </div>
                </div>
                <div class="list-line">
                  {{item.gmtCreate}}<span>
  ...
  export default {
    ...
    methods: {
      ...
      async paperList() {
        ...
        // this.detailData.subjectPic = res.data[0].subjectInfo.subjectPic
        // this.detailData.subjectName = res.data[0].subjectInfo.subjectName
        console.log(this.detailData, '获取试卷列表')

        // let list =[]
        // for(let item of res.data){
        //   let arr = {
        //     subjectName :item.subjectInfo.subjectName,
        //     subjectPic:item.subjectInfo.subjectPic
        //   }
        //   list.push(arr)
        // }
      },
